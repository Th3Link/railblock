...
static void can_receive_task(void *this_ptr) {
    ...
    twai_reconfigure_alerts(TWAI_ALERT_RX_QUEUE_FULL | TWAI_ALERT_ARB_LOST | TWAI_ALERT_BUS_ERROR,
                            NULL);
    ...
    while (!can->shutdown_request()) {
        if (twai_receive(&rx_msg, pdMS_TO_TICKS(100)) == ESP_OK) {
            ...
            if (id_match) {
                can->dispatch(rx_msg.identifier, rx_msg.data, rx_msg.data_length_code, rx_msg.rtr);
            }
        }
        uint32_t alerts;
...
CAN::CAN()
    : m_enable_filter(false), m_update_silence(false), m_bitrate(ICAN::BITRATE_t::BITRATE_50),
      m_id(0xFF), m_type(0xFF), m_rx_pin(GPIO_NUM_NC), m_tx_pin(GPIO_NUM_NC),
      m_shutdown_request(false), m_received(0), m_transmitted(0) {}
...
#define RX_TASK_PRIO 10 // Receiving task priority
    static twai_general_config_t g_config =
        TWAI_GENERAL_CONFIG_DEFAULT(m_tx_pin, m_rx_pin, TWAI_MODE_NORMAL);
    g_config.tx_queue_len = 100;
    g_config.tx_queue_len = 100;
    static const twai_timing_config_t t_config_22_222 = {
        .brp = 200, .tseg_1 = 11, .tseg_2 = 6, .sjw = 3, .triple_sampling = true};
    ...
    g_config.alerts_enabled = TWAI_ALERT_BELOW_ERR_WARN | TWAI_ALERT_RECOVERY_IN_PROGRESS |
    ...
    twai_start();
    xTaskCreatePinnedToCore(can_receive_task, "CAN_rx", 4096, this, RX_TASK_PRIO, NULL,
                            tskNO_AFFINITY);
}
