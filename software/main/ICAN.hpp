...
class ICANDispatcher {
  public:
    virtual bool dispatch(uint32_t identifier, uint8_t *data, unsigned int data_len,
                          bool request) = 0;
};
...
class ICAN {
  public:
    ...
    enum class SILENCE_t : uint8_t { SILENCE_OFF = 0, SILENCE_ON = 1 };
    ...
    enum class ROLLERSHUTTER_MODE_t : uint8_t { SOFTWARE = 1, HARDWARE = 2 };
...
#pragma pack(pop)
    ...
    virtual void send(MSG_ID_t, uint8_t *data, unsigned int data_len, bool request) = 0;
    virtual void send(uint32_t, uint8_t *data, unsigned int data_len, bool request) = 0;
