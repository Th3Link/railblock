...
Relais::Relais(ICAN &ic, PinConfig::signal_config_t signals) : m_can(ic), m_signals(signals) {
...
void Relais::state(uint8_t p_num, bool p_state) {
    if (p_num < 7) {
        gpio_set_level(m_signals.out[p_num], p_state ? 1 : 0);
    }
}

bool Relais::state(uint8_t p_num) {
    return (m_state & (1 << p_num));
}

bool Relais::dispatch(uint32_t identifier, uint8_t *data, unsigned int data_len, bool request) {

    if (static_cast<ICAN::MSG_ID_t>((identifier >> 8) & 0xFF) != ICAN::MSG_ID_t::RELAIS) {
        return false;
    }

    ESP_LOGI(TAG, "Receive Relais Msg: %lx", identifier);
    union {
    ...
    if (request) {
        ...
        ESP_LOGI(TAG, "Relais %x state: %x", relais.number, relais.state);
        state(relais.number, relais.state);
    }
    return true;
}
